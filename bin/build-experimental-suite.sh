...
for mut_rate in 1e-06 1e-08; do
    subtitle="m${mut_rate}"
    python ${BPP_SCRIPT} --title ${subtitle} --population-size 100000 --num-individuals-per-population 4 --num-loci-per-individual 10 --num-characters-per-locus 1000 --mutation-rate-per-site ${mut_rate} ../01-trees/*.lineages.tre || exit
done
