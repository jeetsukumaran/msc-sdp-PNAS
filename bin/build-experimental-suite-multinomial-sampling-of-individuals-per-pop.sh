...
BPP_SCRIPT="${SCRIPT_DIR}/e02b_generate_bpp_analyses_multinomial_sampling_of_individuals_per_population.py"
CUR_DIR=${PWD}
...
for birth_rate in 0.5 ; do
    for extinction_rate in 0.0 0.2 ; do
        ...
        # for conversion_rate in 0000.001 0001.000 1000.000; do
            for sim_time in 5 ; do
...
# for mut_rate in 1e-08; do
    subtitle="m${mut_rate}"
    python ${BPP_SCRIPT} --title ${subtitle} --population-size 100000 --total-number-of-individuals 50 --num-loci-per-individual 10 --num-characters-per-locus 1000 --mutation-rate-per-site ${mut_rate} ../01-trees/*.lineages.tre || exit
done
