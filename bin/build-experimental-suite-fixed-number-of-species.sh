...
for birth_rate in 0.5 ; do
    for extinction_rate in 0.0 0.2 ; do
        for conversion_rate in 0000.001 0000.100 0001.000 0010.000 1000.000; do
            for num_species in 8 ; do
                title="b${birth_rate}_e${extinction_rate}_c${conversion_rate}_n${num_species}"
                echo $title
                python ${TREE_SCRIPT} --title ${title} --b1 ${birth_rate} --b2 ${birth_rate} --c1 ${conversion_rate} --e1 ${extinction_rate} --e2 ${extinction_rate} --max-extant-orthospecies ${num_species} --nreps 20 || exit
            done
        done
    done
done
